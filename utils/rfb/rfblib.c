/**********************************************************************
 * $Id: rfblib.c 50 2012-04-18 20:51:33Z giroudon $
 *
...
#define eprintf(args...) fprintf(stderr, "rfb-client (rfblib): " args)
...
static int rfb_Report_ServerInfo(rfb_t *rfb, FILE *f, char *hdr)
{
  ...
  /* Construct peer IP address string */
  snprintf(peer_addr, sizeof(peer_addr), "%d.%d.%d.%d:%d",
	  (int) (rfb->iremote.sin_addr.s_addr >>  0) & 0xFF,
	  (int) (rfb->iremote.sin_addr.s_addr >>  8) & 0xFF,
	  (int) (rfb->iremote.sin_addr.s_addr >> 16) & 0xFF,
	  (int) (rfb->iremote.sin_addr.s_addr >> 24) & 0xFF,
	  ntohs(rfb->iremote.sin_port) & 0xFFFF);
...
static int rfb_Read(rfb_t *rfb, void *buf, int size)
{
  ...
  if ( disconnected ) {
    eprintf("Connection closed by RFB server\n");
    count = -1;
  }
  else if ( ret == -1 ) {
    perror("RFB read");
...
static int rfb_Authentication(rfb_t *rfb, char *passwd)
{
  ...
  case rfbConnFailed:
    {
      ...
      if ( rfb_DebugFlag )
        eprintf("Authentication Scheme: CONNECTION FAILED\n");
  ...
  case rfbNoAuth:
    if ( rfb_DebugFlag )
      eprintf("Authentication Scheme: NO AUTHENTICATION\n");
    break;
  ...
  case rfbVncAuth:
    {
      ...
      if ( rfb_DebugFlag )
        eprintf("Authentication Scheme: VNC AUTHENTICATION\n");

      /* Check a password is provided */
      if ( passwd == NULL ) {
        eprintf("Password required by RFB server\n");
        return -1;
      }
      ...
      case rfbVncAuthTooMany:
        if ( rfb_DebugFlag )
          eprintf("  Too many authentication failures occured: please wait before reconnecting\n");
        ret = -1;
        break;
      default :
        eprintf("Unknown Authentication Response: %lu\n", response);
        ret = -1;
      }
    }
    break;
  ...
  default :
    eprintf("Unknown Authentication Scheme: %d\n", scheme);
    ret = -1;
  }
...
rfb_t *rfb_New(char *host, int port, char *passwd, int shared)
{
  ...
  if ( rfb_DebugFlag )
	  eprintf("RFB types size: CARD8(%zu) CARD16(%zu) CARD32(%zu)\n",
...
int rfb_PointerScroll(rfb_t *rfb, rfb_scroll_dir_t direction)
{
  ...
  case RFB_SCROLL_UP:
    mask = RFB_BUTTON_UP;
    break;
  case RFB_SCROLL_DOWN:
    mask = RFB_BUTTON_DOWN;
    break;
  case RFB_SCROLL_LEFT:
    mask = RFB_BUTTON_LEFT;
    break;
  case RFB_SCROLL_RIGHT:
